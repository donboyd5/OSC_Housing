...
The tables below show the prevalence of housing insecurity problems in different areas of the country and New York by income group. In each case, households are classified according to their income relative to the median income in the metropolitan area, adjusted for average family size in the area. Median income is estimated by the U.S. Department of Housing and Urban Development.[^who_has_housing_insecurity-1]

[^who_has_housing_insecurity-1]: The measure is known as Housing and Urban Development Area Median Family Income (HAMFI). HUD makes adjustments in their calculation of area median income, and the results differ somewhat from Census-reported median incomes.

In the table comparing states, households are classified according to their income relative to their state's HUD-estimated median income. For the regions of New York, households are classified according to their income relative to the median income in the metropolitan area their county is in, and then the counties are aggregated to the regions of New York.
...
About 80 percent of New York households with income below 30 percent of the area median have some form of housing insecurity, and 73 percent of households with income that is only 30-50 percent of the area median are housing-insecure. The percentages are much smaller for households between 50 and 100 percent of the median, and only 11 percent of households above area median income face housing insecurity. These general patterns hold in most states.
...
tab <- tabdata_topn |> 
  ...
  tab_spanner(columns = c(le30, gt30le50, gt50le80, gt80le100,  gt100),
                label=html("Income relative to area median income adjusted for family size")) |>
...
# tabfile <-  path(oscdir, "problems_income_states.png")
# gtsave(tab, tabfile, zoom=2, expand=20)

tabfile <- tab_save("problems_income_states", tabdata_topn, tab)
...
Lower-income households have the highest rates of housing insecurity in all of New York's regions - particularly households with incomes below 30 percent of the area median. Prevalence of housing insecurity is somewhat lower in New York's upstate regions than in downstate.
...
# tabfile <-  path(oscdir, "problems_income_regions.png")
# gtsave(tab, tabfile, zoom=2, expand=20)

tabfile <- tab_save("problems_income_regions", tabdata_rgn, tab)
...
saveRDS(tabdata, here::here("report", "results", "who_race_tabdata.rds")) 

```

Housing insecurity in New York and in other states is far more common among households headed by a person of color than for households with a white head of household. In New York, 56.1 percent of households headed by an Hispanic person (any race) had at least one housing insecurity problem, compared with 31.2 percent of households headed by a white person; 50.6 percent of households headed by a black or African American person and 48.2 percent of households headed by an Asian person had at least one housing insecurity problem. (See the appendix for state-by-state details.)

This pattern generally holds in all regions of the state.
...
tabfile <-  path(oscdir, "problems_race_regions.png")
gtsave(tab, tabfile, zoom=2, expand=20)

tabfile <- tab_save("problems_race_regions", tabdata_rgn, tab)
...
In New York, 44 percent households headed by a person age 75 face housing insecurity, compared to 39 percent or less for younger households. This is generally true in northeastern states but is not the case in many other states.
...
# tabfile <-  path(oscdir, "problems_age_states.png")
# gtsave(tab, tabfile, zoom=2, expand=20)

tabfile <- tab_save("problems_age_states", tabdata_st, tab)
...
Older households are more likely to be housing-insecure in New York's downstate region but not in the Mohawk Valley or North Country.
...
# tabfile <-  path(oscdir, "problems_age_regions.png")
# gtsave(tab, tabfile, zoom=2, expand=20)

tabfile <- tab_save("problems_age_regions", tabdata_rgn, tab)
